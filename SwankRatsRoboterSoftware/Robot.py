...
class Robot:
    def setLeftMotor(self, percent):
        ...
        if percent == 0:
            ...
            DMCC.setMotor(0, 1, int(percent * 100 * 0.60))
        if percent > 0:
            actuel = 10
            while actuel <= percent:
                print('leftMotor ' + str(actuel))
                DMCC.setMotor(0, 1, int(actuel * 100 * 0.60))
                actuel += 5
                time.sleep(0.30)
        if percent < 0:
            actuel = -10
    ...
    def setRightMotor(self, percent):
        ...
        if percent == 0:
            ...
            DMCC.setMotor(0, 2, int(percent * 100 * 0.60))
        if percent > 0:
            actuel = 10
            while actuel <= percent:
                print('rightMotor ' + str(actuel))
                DMCC.setMotor(0, 2, int(actuel * 100 * 0.60))
                actuel += 5
                time.sleep(0.30)
        if percent < 0:
            actuel = -10
